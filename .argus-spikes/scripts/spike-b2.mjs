...
const JSONL = LOG_DIR + '\\spike-b2-messages.jsonl';
writeFileSync(JSONL, '');

const CHOSEN = 'Alpha'; // the option THIS callback selects
const HOLD_MS = 8000; // short hold: this run only confirms gating+availability without bare allowedTools
const t0 = Date.now();
...
try {
  const q = query({
    ...
    options: {
      ...
      maxTurns: 4,
      allowedTools: [], // no bare name -> must fall through to canUseTool
      cwd: CWD,
      canUseTool,
    },
  });
...
const summary = {
  ...
  usage: result?.usage,
  resolvedInput,
  VERDICT: (holdAchieved >= 7 && hasCodeword && hasChosen) ? 'PASS' : 'FAIL',
};
log(summary);
